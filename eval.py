...
def eval_model(model,dataloader,exp_const):
    ...
    if exp_const.feedforward==False:
        model.AttributeEmbeddings.eval()
    ...
    for it,data in enumerate(tqdm(dataloader)):
        ...
        if exp_const.feedforward==True:
            ...
            class_weights = model.AttributeEmbeddings()
            logits = model.AttributeEmbeddings.classify(feats,class_weights)
...
def main(exp_const,data_const,model_const):
    ...
    if exp_const.feedforward==False:
        model.AttributeEmbeddings = AttributeEmbeddings(model.const.AttributeEmbeddings)
        if model.const.model_num is not None:
